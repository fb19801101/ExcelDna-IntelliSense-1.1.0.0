...
namespace ExcelDna.IntelliSense
{
    ...
    class IntelliSenseDisplay : IDisposable
    {
        ...
        public IntelliSenseDisplay(SynchronizationContext syncContextMain, UIMonitor uiMonitor)
        {
            ...
            Debug.Print(string.Format("### Thread creating IntelliSenseDisplay: Managed {0}, Native {1}", Thread.CurrentThread.ManagedThreadId, Thread.CurrentThread.ManagedThreadId));
        ...
        void InitializeOptions()
        {
            ...
            if (XlCallInt.TryExcel(XlCallInt.xlfGetWorkspace, out result, 37) == XlCallInt.XlReturn.XlReturnSuccess)
            {
                ...
                if (options != null)
                {
                    ...
                    Logger.Initialization.Verbose(string.Format("InitializeOptions - Set ListSeparator to {0}",listSeparator));
                }
            }
            ...
            if (XlCallInt.TryExcel(XlCallInt.xlfGetWorkspace, out result, 56) == XlCallInt.XlReturn.XlReturnSuccess)
            {
                ...
                Logger.Initialization.Verbose(string.Format("InitializeOptions - Set StandardFontName to {0}",standardFontName));
            }
            if (XlCallInt.TryExcel(XlCallInt.xlfGetWorkspace, out result, 57) == XlCallInt.XlReturn.XlReturnSuccess)
            {
                ...
                Logger.Initialization.Verbose(string.Format("InitializeOptions - Set StandardFontSize to {0}",standardFontSize));
            }
        ...
        bool ShouldProcessFormulaEditTextChange(string formulaPrefix)
        {
            ...
            // Not interested...
            Debug.Print(string.Format("Not processing formula {0}",formulaPrefix));
            return false;
        }
        #endregion
        ...
        {
            Debug.Print(string.Format("STATE UPDATE ({0}): \r\t\t\t{1} \r\t\t=>\t{2}", update.Update, update.OldState, update.NewState));
        ...
        {
            Debug.Print(string.Format("IntelliSenseDisplay - FormulaEditStart - FormulaEditWindow: {0}, ArgumentsToolTip: {1}", _formulaEditWindow, _argumentsToolTip));
        ...
        void FormulaEditMove(Rect editWindowBounds, IntPtr excelToolTipWindow)
        {
            ...
            {
                Logger.Display.Warn(string.Format("IntelliSenseDisplay - FormulaEditMove Error - {0}",ex));
        ...
        {
            Debug.Print(string.Format("^^^ FormulaEditStateChanged. CurrentPrefix: {0}, Thread {1}",formulaPrefix,Thread.CurrentThread.ManagedThreadId));
            string functionName;
            int currentArgIndex;
            if (FormulaParser.TryGetFormulaInfo(formulaPrefix, out functionName, out currentArgIndex))
            {
                ...
                if (_functionInfoMap.TryGetValue(functionName, out functionInfo))
                {
                    ...
                    if (_argumentsToolTip != null)
                    {
                        ...
                        {
                            Logger.Display.Warn(string.Format("IntelliSenseDisplay - FormulaEditTextChange Error - {0}",ex));
                            _argumentsToolTip.Dispose();
                            _argumentsToolTip = null;
                        }
                    }
        ...
        void FormulaEditExcelToolTipShow(Rect editWindowBounds, IntPtr excelToolTipWindow)
        {
            ...
            if (_argumentsToolTip != null && _argumentsToolTip.Visible)
            {
                ...
                {
                    Logger.Display.Warn(string.Format("IntelliSenseDisplay - FormulaEditExcelToolTipShow Error - {0}",ex));
                    _argumentsToolTip.Dispose();
                    _argumentsToolTip = null;
                }
            }
        ...
        void FunctionListMove(Rect selectedItemBounds, Rect listBounds)
        {
            ...
            {
                Logger.Display.Warn(string.Format("IntelliSenseDisplay - FunctionListMove Error - {0}",ex));
                // Recycle the _DescriptionToolTip - won't show now, but should for the next function
                _descriptionToolTip.Dispose();
                _descriptionToolTip = null;
            }
        ...
        List<FunctionInfo.ArgumentInfo> GetExpandedArgumentList(FunctionInfo functionInfo, int currentArgIndex)
        {
                ...
                functionInfo.ArgumentList[functionInfo.ArgumentList.Count - 2].Name.EndsWith("1")) // Note: Need both the Arg1 and the ... to trigger the expansion in the function wizard?
            {
                ...
                newList.Add(new FunctionInfo.ArgumentInfo { Name = string.Format("[{0}]",paramsBaseName + 1), Description = paramsDesc });

                // Now if we're at Arg4 (currentParamsIndex=3) then add "[Arg2],[Arg3],[Arg4],[Arg5],..."
                for (int i = 2; i <= currentParamsArgIndex + 2; i++)
                {
                    newList.Add(new FunctionInfo.ArgumentInfo { Name = string.Format("[{0}]",paramsBaseName + i), Description = paramsDesc });
                }
