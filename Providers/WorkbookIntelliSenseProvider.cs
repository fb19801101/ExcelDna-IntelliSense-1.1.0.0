    ...
    class WorkbookIntelliSenseProvider : IIntelliSenseProvider
    {
        ...
        class WorkbookRegistrationInfo
        {
            ...
            public void Refresh()
            {
                ...
                Logger.Provider.Verbose(string.Format("WorkbookRegistrationInfo.Refresh - Workbook {0} at {1}",_name,_path));
                try
                {
                    ...
                    Logger.Provider.Verbose(string.Format("WorkbookRegistrationInfo.Refresh - Read {0} registrations",_regInfo.GetLength(0) - 1));
                }
                catch (Exception ex)
                {
            ...
            public IEnumerable<FunctionInfo> GetFunctionInfos()
            {
                ...
                {
                    Logger.Provider.Warn(string.Format("WorkbookIntelliSenseProvider - Invalid FunctionInfo Identifier: ({0})", idCheck));
                    yield break;
                }
        ...
        public void Initialize()
        {
            ...
            lock (_workbookRegistrationInfos)
            {
                ...
                {
                    var name = wb.Name;
                    Logger.Provider.Verbose(string.Format("WorkbookIntelliSenseProvider.Initialize - Adding registration for {0}",name));
                ...
                for (int i = 0; i < loadedAddIns.GetLength(1); i++)
                {
                    ...
                    Logger.Provider.Verbose(string.Format("WorkbookIntelliSenseProvider.Initialize - Checking Add-In {0}",addInName));
                    if (addInName != null && Path.GetExtension(addInName) != ".xll")    // We don't actually expect the .xll add-ins here - and they're taken care of elsewhere
                    {
                        ...
                        Logger.Provider.Verbose(string.Format("WorkbookIntelliSenseProvider.Initialize - Adding registration for add-in {0}",name));
        ...
        void Excel_WorkbookOpen(Workbook wb)
        {
            ...
            {
                Logger.Provider.Error(ex, string.Format("Unhandled exception in {0}, Workbook: {1}","Excel_WorkbookOpen",name));
            }
        }
        ...
        void Excel_WorkbookBeforeClose(Workbook wb, ref bool cancel)
        {
            ...
            {
                Logger.Provider.Error(ex, string.Format("Unhandled exception in {0}, Workbook: {1}","Excel_WorkbookBeforeClose",name));
            }
        ...
        void Excel_WorkbookAddinInstall(Workbook wb)
        {
            ...
            {
                Logger.Provider.Error(ex, string.Format("Unhandled exception in {0}, Workbook: {1}","Excel_WorkbookAddinInstall",name));
            }
        }
        ...
        void Excel_WorkbookAddinUninstall(Workbook wb)
        {
            ...
            {
                Logger.Provider.Error(ex, string.Format("Unhandled exception in {0}, Workbook: {1}", "Excel_WorkbookAddinUninstall", name));
            }
        }
        ...
        public void Dispose()
        {
            ...
            {
                Logger.Provider.Verbose(string.Format("WorkbookIntelliSenseProvider.Dispose Error {0}",ex));
            }
        }

        string GetXmlPath(string wbPath)
        {
            return Path.ChangeExtension(wbPath, ".intellisense.xml");
        }
    }
}
