...
namespace ExcelDna.IntelliSense
{
    ...
    class LoaderNotification : IDisposable
    {
        ...
        public LoaderNotification()
        {
            ...
            if (status != 0)
            {
                Debug.Print(string.Format("@@@@ LoaderNotification Result: {0}",status));
                throw new InvalidOperationException(string.Format("Error in LdrRegisterDlLNotification. Result: {0}",status));
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ...
                if (_cookie != IntPtr.Zero)
                {
                    ...
                    Logger.Initialization.Verbose(string.Format("LoaderNotification LdrUnregisterDllNotification Result: {0}",status));
                }
                disposedValue = true;
            }
        }
