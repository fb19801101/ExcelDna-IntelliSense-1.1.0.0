...
namespace ExcelDna.IntelliSense
{
    ...
    class ExcelDnaIntelliSenseProvider : IIntelliSenseProvider
    {
        class XllRegistrationInfo
        {
            ...
            public void Refresh()
            {
                ...
                if (regInfoResponse.Equals(ExcelError.ExcelErrorNA))
                {
                    ...
                    Logger.Provider.Info(string.Format("XllRegistrationInfo not available for {0}",_xllPath));
                    return;
                }
            ...
            public IEnumerable<FunctionInfo> GetFunctionInfos()
            {
                ...
                Logger.Provider.Verbose(string.Format("XllRegistrationInfo for {0}: {1} registrations",_xllPath,regInfoCount));
        ...
        {
            Debug.Print(string.Format("@>@>@>@> LoadNotification: {0} - {1}", e.Reason, e.FullDllName));
        ...
        string GetXmlPath(string xllPath)
        {
            return Path.ChangeExtension(xllPath, ".intellisense.xml");
        }
        ...
        IEnumerable<string> GetLoadedXllPaths()
        {
            ...
            //if (loadedDocs == null)
            //{
            //    Logger.Provider.Verbose(string.Format("ExcelDnaIntelliSenseProvider.GetLoadedXllPaths - DOCUMENTS(2) failed");
            //    yield break;
            //}
