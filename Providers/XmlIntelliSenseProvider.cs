...
namespace ExcelDna.IntelliSense
{
    ...
    class XmlIntelliSenseProvider : IIntelliSenseProvider
    {
        public class XmlRegistrationInfo
        {
            ...
            public void Refresh()
            {
                ...
                {
                    Logger.Provider.Warn(string.Format("XmlIntelliSenseProvider - Loading IntelliSense from file {0} failed:\r\n\t{1}", _fileName, ex.Message));
        ...
        public void RegisterXmlFunctionInfo(string fileName, string xmlIntelliSense = null)
        {
            ...
            {
                Logger.Provider.Verbose(string.Format("XmlIntelliSenseProvider.RegisterXmlFunctionInfo - Not IntelliSense file at {0}",fileName));
                return;
            }
            
            var regInfo = new XmlRegistrationInfo(fileName, xmlIntelliSense);
            Logger.Provider.Verbose(string.Format("XmlIntelliSenseProvider.RegisterXmlFunctionInfo - Created XmlRegistrationInfo info for {0}",fileName));
        ...
        {
            Logger.Provider.Verbose(string.Format("XmlIntelliSenseProvider.GetFunctionInfos - Enter on Thread {0}",Thread.CurrentThread.ManagedThreadId));
    ...
    public class XmlIntelliSense
    {
        ...
        public static XmlIntelliSense Parse(string xmlFunctionInfo, string logInfo)
        {
            ...
            {
                Logger.Provider.Warn(string.Format("XmlIntelliSense - Parsing Xml IntelliSense from {0} failed:\r\n{1}",logInfo,ex.Message));
                return Empty;
            }
        }
        ...
        public static XmlIntelliSense Empty { get {return new XmlIntelliSense { XmlFunctionInfo = new XmlFunctionInfo { FunctionsList = new List<FunctionInfo>() } };}}
