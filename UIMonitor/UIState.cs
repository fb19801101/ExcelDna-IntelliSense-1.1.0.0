...
namespace ExcelDna.IntelliSense
{
    ...
    abstract class UIState
    {
        ...
        {
            public IntPtr SelectDataSourceWindow {get;set;}
        }

        public override string ToString()
        {
            #if DEBUG
                //return string.Format("{GetType().Name}{((this is Ready) ? "" : "\r\n")}{string.Join("\r\n", GetType().GetFields().Select(fld => string.Format("\t{fld.Name}: {fld.GetValue(this)}"))}";
                return string.Format("{0}{1}{2}",GetType().Name, (this is Ready) ? "" : "\r\n", string.Join("\r\n", GetType().GetFields().Select(fld => string.Format("\t{0}: {1}",fld.Name, fld.GetValue(this)))));
            #else
        ...
        {
            #if DEBUG
                //return string.Format("{GetType().Name}{((this is Ready) ? "" : "\t")}{string.Join("\t", GetType().GetFields().Select(fld => string.Format("\t{fld.Name}: {fld.GetValue(this)}"))}";
                return string.Format("{0}{1}{2}", GetType().Name, (this is Ready) ? "" : "\t", string.Join("\t", GetType().GetFields().Select(fld => string.Format("\t{0}: {1}", fld.Name, fld.GetValue(this)))));
            #else
                return ToString();
            #endif
        }
    ...
    class UIStateUpdate : EventArgs
    {
        ...
        public override string ToString()
        {
            return string.Format("{0}: {1} -> {2}", Update, OldState, NewState);
        }
