...
namespace ExcelDna.IntelliSense
{
    ...
    class PopupListWatcher : IDisposable
    {
        ...
        public Rect SelectedItemBounds = Rect.Empty;
        public Rect ListBounds = Rect.Empty;
        public IntPtr PopupListHandle { get {return _hwndPopupList;}}
        ...
        void _windowWatcher_PopupListWindowChanged(object sender, WindowWatcher.WindowChangedEventArgs e)
        {
            switch (e.Type)
            {
                case WindowWatcher.WindowChangedEventArgs.ChangeType.Create:
                    ...
                    {
                        Logger.WindowWatcher.Info(string.Format("PopupList window created: {0}",e.WindowHandle));
                    }
                    else
                    {
                        Logger.WindowWatcher.Warn(string.Format("PopupList window created more than once!? Old: {0}, New: {1}",_hwndPopupList,e.WindowHandle));
                    }
                ...
                case WindowWatcher.WindowChangedEventArgs.ChangeType.Destroy:
                    ...
                    Logger.WindowWatcher.Info(string.Format("PopupList window destroyed: {0}",e.WindowHandle));
                    break;
        ...
        {
            Logger.WindowWatcher.Verbose(string.Format("PopupList PopupListElementSelectedHandler on thread {0}",Thread.CurrentThread.ManagedThreadId));
            UpdateSelectedItem();
        }
