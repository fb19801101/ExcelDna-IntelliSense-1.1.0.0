    ...
    class FormulaEditWatcher : IDisposable
    {
        ...
        void SetEditWindow(IntPtr newWindowHandle, ref IntPtr hwnd)
        {
            if (hwnd != newWindowHandle)
            {
                if (hwnd == IntPtr.Zero)
                {
                    Logger.WindowWatcher.Info(string.Format("FormulaEdit SetEditWindow - Initialize {0}",newWindowHandle));    // Could be change of Excel window .... ?
                }
                else
                {
                    Logger.WindowWatcher.Info(string.Format("FormulaEdit SetEditWindow - Change from {0} to {1}",hwnd,newWindowHandle));
                }
                ...
                if (_formulaEditFocus != FormulaEditFocus.None)
                {
                    try
                    {
                        ...
                        Logger.WindowWatcher.Verbose(string.Format("FormulaEdit Uninstalled event handlers for {0}",hwnd));
                    }
                    catch (Exception ex)
                    {
                        Logger.WindowWatcher.Warn(string.Format("FormulaEdit Error uninstalling event handlers for {0}: {1}",hwnd,ex));
                    }
                }
            }
            else
        ...
        //{
        //    // Debug.Print(string.Format(">>>> FormulaEditWatcher.TextChanged on thread {Thread.CurrentThread.ManagedThreadId}");
        //    Logger.WindowWatcher.Verbose(string.Format("FormulaEdit - Text changed in {(sender.Equals(_formulaBar) ? "FormulaBar" : (sender.Equals(_inCellEdit) ? "InCellEdit" : "UNKNOWN"))}");
        ...
        {
            Logger.WindowWatcher.Verbose(string.Format("> FormulaEdit UpdateEditState - Thread {0}",Thread.CurrentThread.ManagedThreadId));
            Logger.WindowWatcher.Verbose(string.Format("FormulaEdit UpdateEditState - Focus: {0} Window: {1}",_formulaEditFocus,(_formulaEditFocus == FormulaEditFocus.FormulaBar ? _hwndFormulaBar : _hwndInCellEdit)));
            ...
            if (hwnd != IntPtr.Zero)
            {
                ...
                Logger.WindowWatcher.Verbose(string.Format("FormulaEdit UpdateEditState Formula editing: CurrentPrefix {0}, EditWindowBounds: {1}", CurrentPrefix, EditWindowBounds));
            }
        ...
        void OnStateChanged(StateChangeType stateChangeType)
        {
            ...
            switch (stateChangeType)
            {
                ...
                case StateChangeType.TextChange:
                    ...
                    break;
                default:
                    throw new ArgumentOutOfRangeException("stateChangeType");
