...
//        public event EventHandler<WindowChangedEventArgs> SelectDataSourceWindowChanged;
        ...
        public WindowWatcher(SynchronizationContext syncContextAuto, SynchronizationContext syncContextMain)
        {
            ...
            // Debug.Print(string.Format("### WindowWatcher created on thread: Managed {Thread.CurrentThread.ManagedThreadId}, Native {AppDomain.GetCurrentThreadId()}");
        ...
        bool UpdateFocus(IntPtr windowHandle, string windowClassName)
        {
            ...
            Logger.WindowWatcher.Verbose(string.Format("Focus lost by {0} ({1})",_focusedWindowHandle,_focusedWindowClassName));
            ...
            Logger.WindowWatcher.Verbose(string.Format("Focus changed to {0} ({1})", windowHandle, windowClassName));
            return true;
        }
            ...
            switch (className)
            {
                ...
                case _excelToolTipClass:
                    ExcelToolTipWindowChanged.Invoke(this, new WindowChangedEventArgs(e.WindowHandle, e.EventType, e.ObjectId));
                     break;
                //case _nuiDialogClass:
                //    // Debug.Print(string.Format("SelectDataSource {_selectDataSourceClass} Window update: {e.WindowHandle:X}, EventType: {e.EventType}, idChild: {e.ChildId}");
