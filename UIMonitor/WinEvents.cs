...
namespace ExcelDna.IntelliSense
{
    ...
    class WinEventHook : IDisposable
    {
        ...
        {
            _syncContextAuto = syncContextAuto;//throw new ArgumentNullException("syncContextAuto");
            _syncContextMain = syncContextMain;// throw new ArgumentNullException("syncContextMain");
        ...
        void InstallWinEventHook(object _)
        {
            ...
            Logger.WinEvents.Info(string.Format("SetWinEventHook success on thread {0}",Thread.CurrentThread.ManagedThreadId));
        }
        ...
        void UninstallWinEventHook(object _)
        {
            if (_hWinEventHook == IntPtr.Zero)
            {
                Logger.WinEvents.Warn(string.Format("UnhookWinEvent unexpectedly called with no hook installed - thread {0}",Thread.CurrentThread.ManagedThreadId));
                return;
            }

            try
            {
                Logger.WinEvents.Info(string.Format("UnhookWinEvent called on thread {0}",Thread.CurrentThread.ManagedThreadId));
            ...
            {
                Logger.WinEvents.Warn(string.Format("UnhookWinEvent Exception {0}",ex));
            }
            finally
            {
        ...
        public void Dispose()
        {
            ...
            Logger.WinEvents.Info(string.Format("WinEventHook Dispose on thread {0}",Thread.CurrentThread.ManagedThreadId));
