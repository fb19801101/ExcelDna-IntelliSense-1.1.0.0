...
namespace ExcelDna.IntelliSense
{
    ...
    class UIMonitor : IDisposable
    {
        ...
        {
            Logger.Monitor.Verbose(string.Format("!> SelectDataSourceWindowChanged ({0})",args.Type));
            OnStateChanged();
        }
        ...
        void _excelToolTipWatcher_ToolTipChanged(object sender, ExcelToolTipWatcher.ToolTipChangeEventArgs e)
        {
            ...
            Logger.Monitor.Verbose(string.Format("!> ExcelToolTip ToolTipChanged Received: {0} with state: {1}", e, CurrentState.GetType().Name));
        ...
        void OnStateChanged(UIState newStateOrNull = null)
        {
            ...
            // Debug.Print(string.Format("NEWSTATE: {newStateOrNull.ToString()} // {(newStateOrNull is UIState.Ready ? Environment.StackTrace : string.Empty)}");
            ...
            {
                Logger.Monitor.Verbose(string.Format(">> {0}",update.LogString()));
        ...
        //    Debug.Print(string.Format("MULTIPLE STATE UPDATES: {updates.Count}");
