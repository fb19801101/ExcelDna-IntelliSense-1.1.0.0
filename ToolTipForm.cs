...
namespace ExcelDna.IntelliSense
{
    ...
    class ToolTipForm  : Form
    {
        ...
        public ToolTipForm(IntPtr hwndOwner)
        {
            ...
            Debug.Print(string.Format("Created ToolTipForm with owner {0}",hwndOwner));
        }
        ...
        {
            Debug.Print(string.Format("@@@ ShowToolTip - Old TopOffset: {0}, New TopOffset: {1}",_topOffset,topOffset));
            _text = text;
            ...
            if (!Visible)
            {
                Debug.Print(string.Format("ShowToolTip - Showing ToolTipForm: {0} => {1}",linePrefix,_text.ToString()));
            ...
            {
                Debug.Print(string.Format("ShowToolTip - Invalidating ToolTipForm: {0} => {1}", linePrefix, _text.ToString()));
                Invalidate();
            }
        }
        ...
        {
            Debug.Print(string.Format("@@@ MoveToolTip - Old TopOffset: {0}, New TopOffset: {1}", _topOffset, topOffset));
            left += _linePrefixWidth;
        ...
        void LaunchLink(string address)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (parts.Length == 2)
                    {
                        ...
                        {
                            MessageBox.Show(string.Format("The help link could not be activated:\r\n\r\nThe file {0} could not be located.",fileName), "IntelliSense by Excel-DNA", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                        ...
                        {
                            MessageBox.Show(string.Format("The help link could not be activated:\r\n\r\nThe file {0} could not be located.",address), "IntelliSense by Excel-DNA", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show(string.Format("The help link could not be activated:\r\n\r\n{0}",ex.Message), "IntelliSense by Excel-DNA", MessageBoxButtons.OK, MessageBoxIcon.Error);
